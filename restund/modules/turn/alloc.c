...
static int relay_listen(const struct sa *rel_addr, struct allocation *al,
			const struct stun_even_port *even, struct turnd *turnd)
{
	uint32_t i;
	int err = 0;
	...
	for (i=0; i<PORT_TRY_MAX; i++) {
		...
		err = udp_listen_in_range(&al->rel_us, rel_addr, udp_recv, al, turnd->turn_min_udp_alloc_port, turnd->turn_max_udp_alloc_port);
		if (err)
			break;
		...
		err = udp_listen_in_range(&al->rsv_us, &al->rsv_addr, NULL, NULL, turnd->turn_min_udp_alloc_port, turnd->turn_max_udp_alloc_port);
		if (err) {
			al->rel_us = mem_deref(al->rel_us);
			continue;
		}
