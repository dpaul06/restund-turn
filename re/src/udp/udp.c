...
/**
 * Create udp socket in a range and listen on a UDP Socket
 *
 * @param usp       Pointer to returned UDP Socket
 * @param local     Local network address
 * @param rh        Receive handler
 * @param arg       Handler argument
 * @param startPort start port of the range
 * @param endPort   stop port of the range
 *
 * @return 0 if success, otherwise errorcode
 */
int udp_listen_in_range(struct udp_sock **usp, const struct sa *local,
	       udp_recv_h *rh, void *arg, int startPort, int endPort)
{
	struct addrinfo hints, *res = NULL, *r;
	struct udp_sock *us = NULL;
	char addr[64];
	...
	int af, error, err = 0;
        ...
        srand(time(NULL));
        int randomPort = startPort + rand() % (endPort-startPort);
	...
	if (local) {
		...
		(void)re_snprintf(addr, sizeof(addr), "%H",
				  sa_print_addr, local);
//		(void)re_snprintf(serv, sizeof(serv), "%u", sa_port(local));
                (void)re_snprintf(serv, sizeof(serv), "%u", randomPort);
	}
	else {
            (void)re_snprintf(serv, sizeof(serv), "%u", randomPort);
#ifdef HAVE_INET6
